...
<head>
  ...
  <table class ="header">
      ...
      <br /> ${record.subsidiary.address.country}<br /><br />${record.subsidiary.custrecord_business_reg_labelling}
                ...
                <td > <p style="align:center; padding-top: 5px"><span class="title"><b>${record.subsidiary.custrecord_3pl_sub_inv_heading}</b></span></p> </td>
                <td align="left"><p style="margin-top: 15px;">${record.subsidiary.custrecord_3pl_sales_support_details}<br />
                  <#if record.custbody_3pl_split_billing_note?length != 0><br /> <span style="background-color:#ffff00;">${record.custbody_3pl_split_billing_note}</span> </#if></p></td></tr>
	</table>
    ...
    <td>&nbsp;</td>
    <td><b>Date:</b><br /><b>Customer ID:</b><br /><b>Invoice #</b><br /><b>PO NO.:</b><br /><b>VAT Reg. No.:</b><br /><b>Currency:</b></td>
 ...
 <macro id="nlfooter">
            <table class="footer" style="width: 100%;border-collapse: collapse;">
              <tr><td colspan="3"><p style="align:center"><b>${record.custbody_3pl_uk_eu_vat}</b></p></td></tr>
              ...
              <tr><td align="center" colspan="3">
                ...
                <#if record.currency = "EUR">Due to recent banking changes, we are no longer able to accept payment via cheque. Please make payment via bank transfer or via credit card by calling our finance team. We apologise for any inconvenience this may cause.</#if>
                
               <table style="width:100%;align:center;border-collapse: collapse;"><tr>
                 
                  <#if record.currency = "USD">
                    <td style="border: 1px solid #bfbfbf;">${record.subsidiary.custrecord_payment_method_ukusd}</td></#if>
                  <#if record.currency = "EUR">
                    <td style="border: 1px solid #bfbfbf;">${record.subsidiary.custrecord_payment_method_ukeur}</td></#if>
                  <#if record.currency = "AED"> 
                    <td style="border: 1px solid #bfbfbf;">${record.subsidiary.custrecord_payment_method_ukaed}</td></#if>
                  <#if record.currency = "GBP">
                    <td style="border: 1px solid #bfbfbf;">${record.subsidiary.custrecord_3pl_payment_method}</td></#if>
...
<thead>
	...
	<td   Style="align:right "><#if item.custcol_1d_bundle?has_content><#if record.currency == 'EUR'>€<#elseif record.currency == 'GBP'>£</#if>${item.custcol_1d_invamt?string.currency?replace("$","")}<#else>${item.amount}</#if></td>
  ...
  <table class = "summarytotal">
    ...
    <tr>
    <td background-color="#ffffff" colspan="4">&nbsp;</td>
    <td align="right" style = "border-color:black; border-left: 2px; border-top: 2px; border-bottom: 2px;   font-size: 16pt;" ><b>Due Date</b></td>
	<td align="right" style = "border-color:black; border-right:2px; border-top: 2px; border-bottom: 2px;   font-size: 16pt;"><b>${record.duedate}</b></td>
